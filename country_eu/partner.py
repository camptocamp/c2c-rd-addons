...
class res_partner(osv.osv):
    ...
    def check_fiscal_position(self, cr, uid, ids, context):
        ...
        home_country_id = ''
        if company_id and current_user.company_id.partner_id.country_id:
            home_country_id = current_user.company_id.partner_id.country_id.id
        
        for partner in self.browse(cr, uid, ids, context):
            if home_country_id and not partner.property_account_position:
                d = time.strftime('%Y-%m-%d')
                #for address in partner.address:
                if partner.country_id and partner.country_id.id != home_country_id:
                    for country in country_obj.browse(cr, uid, [partner.country_id.id], context):

         ...
         then
            ...
            IF not found
               then
                   ...
                   select move_rec.company_id, move_rec.account_id, move_rec.period_id, debit_p, credit_p, to_char(date_start,'YYYYMM'),
                   get_id('account_account_period_sum',move_rec.company_id,move_rec.account_id,fiscalyear_id,0)
                      from account_period
               ...
               IF not found
                 then
                   insert into account_account_period_sum
                    (company_id,account_id,period_id,debit,credit,name,sum_fy_period_id)
                   values(
                    move_rec.company_id, move_rec.account_id, period_00_rec.period_id, debit_p, credit_p,period_00_rec.code,
                     get_id('account_account_period_sum',move_rec.company_id,move_rec.account_id,period_00_rec.fiscalyear_id,0 ));
               END IF;
           end loop;
...
insert into account_account_period_sum
    (company_id, account_id, period_id, debit, credit, name, sum_fy_period_id)
     ...
     get_id('account_account_period_sum',s.company_id,a.id,y.id,0)
     from account_account a,
 ...
 then
   ...
   insert into account_account_period_sum
    (company_id, account_id,period_id,debit,credit,name,sum_fy_period_id)
     select
     s.company_id,
     a.id, p.id, sum(debit), sum(credit), y.code||'00',
     get_id('account_account_period_sum',s.company_id,a.id,y.id,0)
     from account_account a,
...
# Periodensummen erzeugen
        cr.execute("""
insert into account_account_period_sum(name,sum_fy_period_id,period_id,company_id,account_id,credit,debit)
select   p.name,
         get_id('account_account_period_sum', l.company_id, l.account_id, p.fiscalyear_id,0),
         l.period_id,
         l.company_id,
         account_id,

...
#""")
    ...
    def allow_reopen(self, cr, uid, ids, context=None):
        ...
        for pick in self.browse(cr, uid, ids, context):
            ...
            if pick._columns.get('invoice_ids'):
                _logger.info('FGF picking allow open inv_ids '   )
                ids2 = []
                if pick.invoice_ids:
                ...
                _logger.info('FGF picking invoiced '   )
	...
	self.allow_reopen(cr, uid, ids, context=None)
        ...
        for pick in self.browse(cr, uid, ids):
            _logger.info('FGF picking action reopen pick %s ' %(pick.name)   )
            ml_ids = []
